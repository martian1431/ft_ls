...
/*   Updated: 2019/08/30 08:17:54 by pmalope          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	int		ft_verify_opt(char c, char *str)
{
	int		i;
...
static	void	ft_init_arg(t_option *opt, char *str)
{
	int		i;
	...
	while (str[++i])
	{
		if (ft_verify_opt(str[i], "lRrat") || (str[1] == '-' && !str[2]))
		{
...
static	void	ft_parse_param(int nb, char **arg, t_option *opt, t_list **path)
{
	int		i;
	int		flag;
